...
def message_process(update, context):
    ...
    if(update.message.chat.type == "supergroup"):
        ...
        #if it talks to another bot
        if(update.message.reply_to_message.from_user.id == context.bot.get_me().id and update.message.from_user.is_bot):
            return _ai_reply(update,context,group_id)
        #if for wait time
